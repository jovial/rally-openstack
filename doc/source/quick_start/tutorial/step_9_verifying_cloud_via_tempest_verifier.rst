...
pairs, where the keys are either a regex, to match against the test name,
or a test id. The values are the reason why the test was skipped. If an
invalid regex is supplied the key is treated as a test id. For example:
